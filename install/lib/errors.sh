...
# Trap for critical script failures and user interruption
setup_error_trap() {
    trap 'error_exit "An unexpected error occurred in $(basename "$0") at line $LINENO."' ERR
    trap 'error_exit "Installation interrupted by user." 130' SIGINT
}
...
error_exit() {
    ...
    log "EXIT" "Script exited with code $exit_code: $message"
    exit "$exit_code"
}
