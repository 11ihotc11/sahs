...
info "Installing sahs-brightness script"
run_task "Copying sahs-brightness to /usr/local/bin" sudo cp "$BASE_DIR/bin/sahs-brightness" /usr/local/bin/
run_task "Ensuring script is executable" sudo chmod +x /usr/local/bin/sahs-brightness
...
success "sahs-brightness setup completed"
